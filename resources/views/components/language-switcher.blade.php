...
<div class="relative inline-block text-left" x-data="{ open: false }">
    <div>
        ...
        >
            ...
            <!-- Chevron -->
            <svg class="-{{ LocaleHelper::isRtl() ? 'mr' : 'ml' }}-1 {{ LocaleHelper::isRtl() ? 'mr' : 'ml' }}-2 h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
    ...
    <div 
        ...
        class="origin-top-{{ LocaleHelper::isRtl() ? 'left' : 'right' }} absolute {{ LocaleHelper::isRtl() ? 'left' : 'right' }}-0 mt-2 w-48 rounded-lg shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-50" 
        role="menu" 
        ...
        <div class="py-1" role="none">
            @foreach($locales as $code => $name)
                <a 
                    ...
                    class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900 transition-colors {{ $currentLocale === $code ? 'bg-gray-50 font-semibold' : '' }}" 
                    role="menuitem" 
                    tabindex="-1"
                >
